...
import { MarkupListPage as ShowCaseList } from 'pages/markups';
...
function DashboardPage() {
  ...
  const scriptureStat = useSWR<PaginatedInterface<ScriptureInterface>>(
    () => `/scriptures`,
    () => getScriptures({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    ...
    {
      title: 'Scriptures',
      result: scriptureStat?.data?.totalCount,
      isLoading: scriptureStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
          ...
          <Stack direction="column" spacing={8} flex={{ base: 1, md: 7 }}>
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                ...
                <NextLink href={`markups`} passHref legacyBehavior>
                  <Button
